...
shape_colors = [(255,0,0),(0,255,0),(0,0,255),(0,255,255),(255,255,0),(255,165,0),(128,0,128)]
...
def convert_shape(shape):
	...
	for i, line in enumerate(format):
		...
		for j, column in enumerate(row):
			if column == '0':
				position.append((shape.x + j-2, shape.y + i-4))

	return position

def valid_space(shape, grid):
	valid_postions = [[(j, i) for j in range(10) if grid[i][j] == (0,0,0)] for i in range(20)]
	valid_postions = [i for sub in valid_postions for i in sub]

	formatted = convert_shape(shape)

	for pos in formatted:
		if pos not in valid_postions:
			if pos[1]>-1:
				return False
	return True
...
def clear_rows(grid, locked_pos):
	...
	if inc>0:
		for key in sorted(locked_pos, key= lambda x: x[1])[::-1]:
			x, y = key
			if y < ind:
				new_key = (x, y + inc)
...
def draw_middle(surface, text, size=50, color = (0,0,0)):
...
def main(win):
	...
	while run:
		...
		if check_lost(locked_pos):
			draw_middle(win, "You Lost!", 70, (255,255,255))
			pygame.display.update()
...
def main_menu(win):
	...
	while run:
		...
		draw_middle(win, "Press any key to start playing")
		pygame.display.update()
		for event in pygame.event.get():
			if event.type == pygame.QUIT:
				pygame.display.quit()
